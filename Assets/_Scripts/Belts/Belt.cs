...
private IEnumerator StartBeltMove()
{
    ...
    {
        // Ambil Rigidbody item, kalau gak ada tambahin
        Rigidbody rb = beltItem.item.GetComponent<Rigidbody>();
        if (rb == null)
            rb = beltItem.item.AddComponent<Rigidbody>();

        rb.isKinematic = true; // Matikan physics agar tidak jatuh

        Vector3 toPosition = beltInSequence.GetItemPosition();
        beltInSequence.isSpaceTaken = true;

        float step = _beltManager.speed * Time.deltaTime;

        while (Vector3.Distance(beltItem.item.transform.position, toPosition) > 0.01f)
        {
            rb.MovePosition(Vector3.MoveTowards(beltItem.item.transform.position, toPosition, step));
            yield return null;
        }
